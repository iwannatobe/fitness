...
- **数据备份**：
  - 点击顶部中间 logo → 翻盖露出红色紧急导出键（圆形仪表风）→ 自动弹确认框 → 生成 JSON 备份
  - 统计页"导入历史"按钮 → 文件选择器选备份恢复，重复记录自动跳过
  - 导入弹窗提示"导出数据：点击顶部 logo"
- **性能修复**：
  - 完成训练项切换卡顿修复：完成一项时不再重建资料馆/有氧页大网格，改为切页时惰性刷新，完成耗时从约 600ms 降至 46ms
  - AI 配置保存卡死修复：保存 API Key 后不再在主线程初始化 httpx 客户端，改为后台线程惰性创建
  - 顶栏 AI 状态灯读取配置从每秒改为 10 秒缓存，减少 Android 文件 IO

### v2.1 (2026-08-01)

- **资料馆**：原"力量训练"页改造为动作资料馆，含 333 个常用动作的图文/动画讲解
  - 网格卡片浏览（一行三个），按身体部位（胸/背/肩/手臂/腿/小腿/核心）与"常用"快捷筛选，支持搜索
  - "常用"默认空：完成模板训练的动作自动计入，按最近使用排序
  - 点击动作弹出动画帧 + 中文步骤 + 目标肌群/器械详情（复用离线资料库）
  - 从 1324 动作数据集中精选补齐到 333 个，每个含缩略图、GIF、动画帧序列、中文讲解
- **有氧资料馆**：有氧页改造为资料馆，含 59 个图文动作
  - 分类筛选：有氧运动（18）/ 练前热身（17）/ 练后拉伸（24），支持搜索
  - 删掉原预设中无图文的项目（游泳/骑行/快走/瑜伽等）
  - 每个动作含缩略图、动画帧序列、中文步骤讲解
- **AI 快捷添加**：AI 的 `add_plan_item` 对资料库动作自动关联讲解并写入建议休息时间；`search_catalog` 返回常用标记
- **核弹部署增强**：常用动作及有氧/热身/拉伸动作进入部署弹窗的快捷网格，一键加入今日计划/模板

### v2.0 (2026-07-29)

- **组间恢复倒计时**：右上角 `SYSTEM BUS` 底部的 RST 滚动条点击即开始倒计时，作为完成一组后的休息恢复
  - 每个力量项目按动作类型自动分配恢复时间：复合大重量（深蹲/硬拉/抓举/挺举/推举…）180s，孤立小肌群（弯举/飞鸟/侧平举/腿屈伸…）90s，其他默认 120s
  - 按墙上时钟从左到右逐段点亮，正好滚到最右侧时全段变绿，并播完成提示音
  - 计划弹出窗与热身页可逐项微调休息秒数（步进 15s）
  - 倒计时持久化到当日训练会话，重启应用后进度仍按墙上时钟续算
- **完成提示音**：恢复倒计时归零时播 `rest_complete` 提示音
